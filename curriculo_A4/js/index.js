...
document.addEventListener("DOMContentLoaded", function () {
  ...
  html2canvas(corpoHTML).then((canvas) => {
    const imgData = canvas.toDataURL("image/png");
    const { jsPDF } = window.jspdf;
    const largura = 210; // Largura em mm para o formato A4
    const altura = 297; // Altura em mm para o formato A4
    const x = 10; // Posição x onde a imagem começa
    const y = 10; // Posição y onde a imagem começa
    const doc = new jsPDF("p", "mm", [largura, altura]);
    doc.addImage(imgData, "PNG", x, y, largura - 20, altura - 20); // Deixando uma margem de 10mm em cada lado
